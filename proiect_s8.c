...
void prelucrareFisierBMP(char denumire_fisier1[], char denum[]){
	...
	strcat(denumire_fisier,denum);
	printf("%s    Testarea mea lalalala \n",denumire_fisier);

	
	input = open(denumire_fisier, O_RDWR);
		if(input == -1 )
		{
		...
		int dimensiune_rand = ((size - 54) / size) * 4;
		printf("File size: %lld bytes\n", (long long)file_stat.st_size);
		
		
	for (int i = 54; i < file_stat.st_size; i += 3) {
        // Read RGB values
        uint8_t blue, green, red;
        pread(input, &blue, sizeof(uint8_t), i);
        pread(input, &green, sizeof(uint8_t), i + 1);
        pread(input, &red, sizeof(uint8_t), i + 2);

        // Calculate grayscale value
        uint8_t gray = (uint8_t)((red + green + blue) / 3);

        // Write grayscale values
        pwrite(input, &gray, sizeof(uint8_t), i);
        pwrite(input, &gray, sizeof(uint8_t), i + 1);
        pwrite(input, &gray, sizeof(uint8_t), i + 2);
    }
...
off_t getlungimeafisieruluioriginal(const char *linkPath,const char path[]) {
    ...
    if (stat(resolvedPath, &targetStat) == -1) {
        perror("Error getting target file status");
        exit(0);
    }

    return targetStat.st_size;
}
void procesare_fisier(char denumire_fisier1[], char denum[],int tip, char calea_fisier_output[])

{

int statistica_linii=0;
char denumire_fisier[50];
...
memcpy(denumire_fisier, denumire_fisier1, strlen(denumire_fisier1)+1);
	...
	//dimesinue legatura
		...
		char calea_si_denumire_fisier_output[70];
		memcpy(calea_si_denumire_fisier_output, calea_fisier_output, strlen(calea_fisier_output)+1);
		strcat(calea_si_denumire_fisier_output,denum);
		strcat(calea_si_denumire_fisier_output,"_statistica.txt");
		int fd=open(calea_si_denumire_fisier_output,O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR);
...
{
	printf("Test \n");
	//presupunem ca maxim 10 procese
	pid_t cpid[10];
	static int numar_curent_procese=0;
	...
	stat(entry->d_name, &sb);
    	...
    	char *extensie = dot +1;
    	if(strcmp(extensie,"bmp")==0)  //caz 1
    		{
    	...
    	printf("NUmarCurrent procese %d\n",numar_curent_procese);
	int pid=-1;
	if( ( cpid[numar_curent_procese]=fork() ) < 0)
  	{
    		perror("Eroare");
    		exit(1);
  	}
  	
	else if(cpid[numar_curent_procese]==0)
  	{
  		
    		/* codul fiului - doar codul fiului poate ajunge aici (doar in fiu fork() returneaza 0)*/
    		
    		procesare_fisier(argv[1],entry->d_name,tip,argv[2]);  //functia de procesare
    		// apel necesar pentru a se opri codul fiului astfel incat acesta sa nu execute si codul parintelui
    		exit(1);
  	}
  	numar_curent_procese++;
  	if(tip==1)
  	{
  		//cream mai inca un process pentru prelucrarea fisierului .bmp
  		prelucrareFisierBMP(argv[1],entry->d_name);
  	}
    	...
    	closedir(folder);
    		...
    		for (int i = 0; i < numar_curent_procese; i++) 
    		{
        	int status;
        	pid_t cpidd = waitpid(cpid[i], &status, 0);
